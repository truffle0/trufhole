...
IUSE="abi_mips_n32 audit debug hpn kerberos ldns libedit livecd pam +pie rootprefix +scp sctp security-key selinux +ssl static test X X509 xmss"
...
src_configure() {
	...
	if use rootprefix; then
		econf --prefix="${EPREFIX}/" "${myconf[@]}"
	else
		econf "${myconf[@]}"
	fi
}
