...
src_prepare() {
	...
	use elibc_musl && eapply "${FILESDIR}/openjdk-11-musl-fix-reinterpret-cast.patch"
	use elibc_musl && eapply "${FILESDIR}/openjdk-17-fix-lld-symbols.patch"
	default
...
src_configure() {
	...
	local myconf=(
		...
		$(use elibc_musl && echo "--with-debug-level=release")
		$(use elibc_musl && echo "--with-native-debug-symbols=none")
