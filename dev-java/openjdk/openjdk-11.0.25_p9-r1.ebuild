...
src_prepare() {
	...
	# lld-related symbol fixes
	if tc-is-clang ; then
		PATCHES+=(
			"${FILESDIR}/${PN}-11-lld-fix-missing-symbols.patch"
			"${FILESDIR}/${PN}-11-fix-clang13-build.patch"
		)
	fi
...
src_configure() {
	...
	# backport of fix to libc++ linker error, bug #935803
	# openjdk tries to statically link to libstdc++ by default, regardless of toolchain
