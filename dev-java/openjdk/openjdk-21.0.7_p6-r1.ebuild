...
bootstrap_uri() {
	...
	# here be dragonshttps://github.com/openjdk/jdk21u.git
	echo "${kw}? ( ${cond:+${cond}? (} ${baseuri}-${ver}-${kw}${musl:+-musl}.${suff} ${cond:+) })"
}

# don't change versioning scheme
# to find correct _p number, look at
# https://github.com/openjdk/jdk${SLOT}u/tagshttps://github.com/openjdk/jdk21u.git
...
src_prepare() {
	...
	# bug #922147
	if tc-is-clang ; then
		PATCHES+=( "${FILESDIR}/${PN}-21-lld-fix-missing-symbols.patch" )
	fi
...
src_configure() {
	...
	# bug #922147
	tc-is-clang && append-ldflags -Wl,--undefined-version
